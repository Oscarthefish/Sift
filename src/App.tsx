...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {loading ? <div className="empty"><p>Opening your local ledger…</p></div> : view === "overview" ? <Overview transactions={monthTransactions} spend={spend} personalTotal={personalTotal} havenTotal={havenTotal} reviewCount={reviewCount} opportunities={opportunities} onReview={() => setView("review")} onImport={() => setImportOpen(true)} /> : view === "review" ? <Review transactions={monthTransactions} knownTags={[...new Set(transactions.flatMap((item) => item.tags))]} onUpdate={update} onFindTransfers={() => setTransferSuggestions(suggestTransfers(transactions))} /> : view === "trends" ? <Trends transactions={transactions} /> : <Rules rules={rules} onSave={async (rule) => { await repository.saveRule(rule); await refresh(); setNotice("Rule updated."); }} onDelete={async (id) => { await repository.deleteRule(id); await refresh(); setNotice("Rule deleted. Existing transactions were left unchanged."); }} />}
    </main>
...
function Review({ transactions, knownTags, onUpdate, onFindTransfers }: { transactions: Transaction[]; knownTags: string[]; onUpdate: (item: Transaction, remember?: boolean) => Promise<void>; onFindTransfers: () => void | Promise<void> }) {
  ...
  const [draftTags, setDraftTags] = useState<string[]>(selected?.tags ?? []);
  const [newTag, setNewTag] = useState("");
  const availableTags = [...new Set([...knownTags, ...draftTags])].sort((left, right) => left.localeCompare(right));
  useEffect(() => {
    const next = transactions.find((item) => item.id === selected?.id) ?? transactions[0] ?? null;
    setSelected(next);
    setDraftTags(next?.tags ?? []);
    setNewTag("");
  }, [transactions]);
  const choose = (item: Transaction) => { setSelected(item); setDraftTags(item.tags); setNewTag(""); };
  const toggleTag = (tag: string) => setDraftTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag]);
  const createTag = () => {
    const tag = newTag.trim().replace(/^#/, "");
    if (tag && !draftTags.includes(tag)) setDraftTags((current) => [...current, tag]);
    setNewTag("");
  };
  const save = (remember = false) => selected && onUpdate({ ...selected, tags: draftTags }, remember);
  return <section className="review-layout"><div className="review-list"><div className="review-tools"><span>{transactions.length} transactions</span><button onClick={onFindTransfers}>Find transfers</button></div>{transactions.map((item) => <button key={item.id} className={`review-row ${selected?.id === item.id ? "selected" : ""}`} onClick={() => choose(item)}><span className={!item.categoryId && item.amount < 0 && !item.excludedFromSpending ? "review-dot attention" : "review-dot"} /><span><strong>{item.merchant}</strong><small>{item.postedAt} · {item.accountId.replace("westpac-", "")}</small></span><b>{money.format(Math.abs(item.amount))}</b></button>)}</div>
    <aside className="editor">{selected ? <><p className="eyebrow">Transaction details</p><h2>{selected.merchant}</h2><p className="original">{selected.description}</p><label>Category<select value={selected.categoryId ?? ""} onChange={(event) => setSelected({ ...selected, categoryId: event.target.value || undefined })}><option value="">Needs review</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>Use<select value={selected.context} onChange={(event) => setSelected({ ...selected, context: event.target.value as "personal" | "haven" })}><option value="personal">Personal</option><option value="haven">Haven</option></select></label><div className="tag-field"><span>Tags</span>{availableTags.length > 0 && <div className="tag-chips">{availableTags.map((tag) => <button type="button" className={draftTags.includes(tag) ? "selected" : ""} key={tag} onClick={() => toggleTag(tag)}>{tag}</button>)}</div>}<div className="new-tag"><input value={newTag} placeholder="Create a new tag" onChange={(event) => setNewTag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === ",") { event.preventDefault(); createTag(); } }} /><button type="button" disabled={!newTag.trim()} onClick={createTag}>Add</button></div>{draftTags.length > 0 && <small>Applied: {draftTags.join(", ")}</small>}</div><label className="check"><input type="checkbox" checked={selected.excludedFromSpending} onChange={(event) => setSelected({ ...selected, excludedFromSpending: event.target.checked, exclusionReason: event.target.checked ? "transfer" : undefined })} /> Exclude from spending</label><div className="editor-actions"><button onClick={() => save()}>Save once</button><button className="primary" disabled={!selected.categoryId} onClick={() => save(true)}><Check size={15} /> Save & remember</button></div><small className="privacy-copy">Remembering creates a local merchant rule and updates matching transactions. Nothing leaves this Mac.</small></> : <p>Select a transaction.</p>}</aside></section>;
}
